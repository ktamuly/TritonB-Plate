...
def scaled_dot_product_attention_kernel(
    ...
    q = tl.load(
        q_start + n_offsets[:, None] * stride_qn + d_offsets[None, :] * stride_qd,
        mask=(n_offsets[:, None] < N) & (d_offsets[None, :] < d),
        other=0.0,
    )
    ...
    for k_idx in range(0, N, BLOCK_SEQLEN):
        ...
        k = tl.load(
            k_start + k_offsets[:, None] * stride_kn + d_offsets[None, :] * stride_kd,
            mask=mask,
            other=0.0,
        )
        ...
        v = tl.load(
            v_start + k_offsets[:, None] * stride_vn + d_offsets[None, :] * stride_vd,
            mask=mask,
            other=0.0,
        )
        output += tl.dot(scores_exp, v)
    tl.store(
        out_start + n_offsets[:, None] * stride_on + d_offsets[None, :] * stride_od,
        output,
        mask=(n_offsets[:, None] < N) & (d_offsets[None, :] < d),
    )
