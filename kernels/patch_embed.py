...
def patch_embedding_kernel(
    img_ptr, weight_ptr, out_ptr,
    batch_size, in_channels, height, width,
    patch_size, embedding_dim,
    img_stride_b, img_stride_c, img_stride_h, img_stride_w,
    weight_stride_o, weight_stride_i, weight_stride_h, weight_stride_w,
    out_stride_b, out_stride_p, out_stride_e,
    BLOCK_SIZE: tl.constexpr, BLOCK_E: tl.constexpr,
):
    ...
    for h_offset in range(patch_size):
        for w_offset in range(patch_size):
            ...
            for c in range(in_channels):
                pixel_ptr = img_ptr + pid_batch * img_stride_b + c * img_stride_c + h_pos * img_stride_h + w_pos * img_stride_w
                pixel = tl.load(pixel_ptr)
                
                weight_ptrs = weight_ptr + e_offsets * weight_stride_o + c * weight_stride_i + h_offset * weight_stride_h + w_offset * weight_stride_w
                weights = tl.load(weight_ptrs, mask=e_mask, other=0.0)
                
                acc += tl.where(e_mask, pixel * weights, 0.0)
    
    out_ptr_base = out_ptr + pid_batch * out_stride_b + pid_patch * out_stride_p + e_start * out_stride_e
...
def triton_patch_embedding(img, patch_size, embedding_dim):
    ...
    assert height % patch_size == 0, f"Image height {height} is not divisible by patch size {patch_size}"
    assert width % patch_size == 0, f"Image width {width} is not divisible by patch size {patch_size}"
    ...
    weight = torch.randn(embedding_dim, in_channels, patch_size, patch_size, 
                        device=img.device, dtype=img.dtype) * 0.02
    
    out = torch.empty((batch_size, num_patches, embedding_dim), 
