...
@triton.jit
def linear_kernel(x_ptr, weight_ptr, bias_ptr, out_ptr,
                  B, N, in_features, out_features,
                  stride_xb, stride_xn, stride_xh,
                  stride_wo, stride_wi,
                  stride_ob, stride_on, stride_oh,
                  BLOCK_B: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_O: tl.constexpr):
    ...
    for i_idx in range(0, in_features, 32):
        ...
        x_ptrs = x_ptr + b_offsets[:, None, None] * stride_xb + n_offsets[None, :, None] * stride_xn + i_block[None, None, :] * stride_xh
        x = tl.load(x_ptrs, mask=b_mask[:, None, None] & n_mask[None, :, None] & i_mask[None, None, :], other=0.0)
        w_ptrs = weight_ptr + o_offsets[:, None] * stride_wo + i_block[None, :] * stride_wi
        w = tl.load(w_ptrs, mask=o_mask[:, None] & i_mask[None, :], other=0.0)
        acc += tl.dot(x, w)
    bias = tl.load(bias_ptr + o_offsets, mask=o_mask, other=0.0)
    result = acc + bias
    out_ptrs = out_ptr + b_offsets[:, None, None] * stride_ob + n_offsets[None, :, None] * stride_on + o_offsets[None, None, :] * stride_oh
    tl.store(out_ptrs, result, mask=b_mask[:, None, None] & n_mask[None, :, None] & o_mask[None, None, :])
...
def triton_linear(x, weight, bias=None):
    ...
    assert in_features == in_features_w, f"Input features {in_features} doesn't match weight shape {in_features_w}"
